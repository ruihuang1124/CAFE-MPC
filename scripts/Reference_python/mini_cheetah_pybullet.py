...
HIP_OFFSETS_ARCDOG = np.array([[0.274, 0.065, 0],
                               [0.274, -0.065, 0],
                               [-0.274, 0.065, 0],
                               [-0.274, -0.065, 0]])

DEFAULT_JOINT_POSE_ARCDOG = [0, -0.8, 1.4,
                      0, -0.8, 1.4,
                      0, -0.8, 1.4,
                      0, -0.8, 1.4]
...
class ArcDog:
    ...
    def __init__(self, urdf_file=None):
        ...
        self.urdf_file = urdf_file
        self.robot = self.pb.loadURDF(self.urdf_file)
        self.set_pose(self.INIT_POS, self.INIT_QUAT, DEFAULT_JOINT_POSE_ARCDOG)
    ...
    def fk(self,
           ...
           leg = None, qleg = None):
        pf_in_hip = self.leg_fk(leg, qleg)
        pf_in_body = HIP_OFFSETS_ARCDOG[leg] + pf_in_hip
        quat = self.eul2quat(base_eul)
        rot = np.reshape(self.pb.getMatrixFromQuaternion(quat), (3,3))
        pf_in_world = np.matmul(rot, pf_in_body) + base_pos
